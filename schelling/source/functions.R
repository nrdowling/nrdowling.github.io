...
# 2) responses that *may* reference single locations in multiple ways (e.g., the reg, the library; my room, the a UChi Dorm, Burton Judson)
...
clean_meetup <- function(data, colName, colErr="") {
    ...
    y <- y %>%
        ...
        mutate(x = case_when(x %ilike% "quad" ~ "the Quad",
                             ...
                             x %ilike% "sears|willis" ~ "the Sears/Willis Tower",
                             x %ilike% "reynolds" ~ "the Reynold's Club",
                             #x %ilike% "\\bart\\b" ~ "Art Institute",
                             x %ilike% "\\bex\\b" ~ "Ex Libris",
                             x %ilike% dining ~ "another cafe/dining",
                             x %ilike% bean ~ "the Bean",
                             x %ilike% thereg ~ "the Reg",
                             x %ilike% library ~ "another library",
                             x %ilike% dorm ~ "a UChi Dorm",
                             x %ilike% residence ~ "another residence",
                             x %ilike% recent ~ "a recent meeting location",
                             x %ilike% uc_other ~ "another UChicago site",
                             x %ilike% chi_other ~ "another Chicago landmark",
                             x %ilike% mapssbld ~ "the 1155 Building",
                             TRUE ~ "Other"))
    ...
    # students & friends you want to name more libraries or specify a UChi Dorm but collapse all off-campus
    ...
    # } else if (...) {...}
    if (colName %ilike% "stranger|student") {
        y <- y %>%
...
pie_meetup <- function(data, colName, levDisplay=5, show_n = TRUE, title="", caption="") {
